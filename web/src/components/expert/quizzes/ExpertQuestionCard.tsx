...
export default function ExpertQuestionCard({
  question,
  caseThumbnail,
  onEdit,
  onDelete,
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
          <div className="relative aspect-video overflow-hidden rounded-2xl bg-black">
            <>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={caseThumbnail || `/api/placeholder/320/180`}
                alt={question.caseTitle || 'Placeholder diagnostic image'}
                className="h-full w-full object-cover opacity-95"
              />
      ...
      <div className="flex gap-5">
        <div className="relative h-32 w-44 flex-shrink-0 overflow-hidden rounded-xl bg-muted">
          <>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={caseThumbnail || `/api/placeholder/176/128`}
              alt={question.caseTitle || 'Diagnostic placeholder'}
              className="h-full w-full object-cover opacity-95"
            />
            <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity group-hover:opacity-100">
